...
void ESP_CAMERA::init_stream_camera()
{
...
void ESP_CAMERA::init_video_camera()
{
  camera_config_t config;
  config.ledc_channel = LEDC_CHANNEL_0;
  config.ledc_timer = LEDC_TIMER_0;
...
int ESP_CAMERA::get_frame(byte *frame, bool add_headers, int num)
{
  int bytes = -1;
  ...
  if (!fb)
  {
    ...
    {
      fs::FS &tfs = SD_MMC;
      String imageName = "/frame" + String(num) + ".jpg";
      File file = tfs.open(imageName.c_str(), FILE_WRITE);
